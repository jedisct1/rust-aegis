...
impl State {
    ...
    fn stream(&mut self, out: &mut [u8]) {
        let (blocks, last) = out.as_chunks_mut::<16>();
        for block in blocks {
            self.enc(block, &[0u8; 16]);
        }
        last.fill(0);
    ...
    fn stream_xor(&mut self, mc: &mut [u8]) {
        let (blocks, last) = mc.as_chunks_mut::<16>();
        for block in blocks {

...
/// AEGIS-128X4 exposed through the RustCrypto `aead` 0.6 traits.
///
...
impl AeadCore for Aegis128X4<16> {
    type NonceSize = U16;
    type TagSize = U16;
    const TAG_POSITION: aead::TagPosition = aead::TagPosition::Postfix;
}

impl AeadCore for Aegis128X4<32> {
    type NonceSize = U16;
    type TagSize = U32;
    const TAG_POSITION: aead::TagPosition = aead::TagPosition::Postfix;
}

impl<const TAG_SIZE: usize> KeySizeUser for Aegis128X4<TAG_SIZE> {
    type KeySize = U16;
}

impl<const TAG_SIZE: usize> KeyInit for Aegis128X4<TAG_SIZE> {
    fn new(key: &Array<u8, Self::KeySize>) -> Self {
        Self { key: *key }
    }
}

impl AeadInOut for Aegis128X4<16> {
...
impl AeadInOut for Aegis128X4<32> {
    fn encrypt_inout_detached(
        &self,
        nonce: &Nonce<Self>,
        associated_data: &[u8],
...
fn test_wrappers() {
    let key = [0u8; 16];
    let nonce = [0u8; 16];
    let state = Aegis128X4::<16>::new(&key.into());

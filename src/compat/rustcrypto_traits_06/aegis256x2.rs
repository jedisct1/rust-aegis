...
/// AEGIS-256X2 exposed through the RustCrypto `aead` 0.6 traits.
///
...
impl AeadCore for Aegis256X2<16> {
    type NonceSize = U32;
    type TagSize = U16;
    const TAG_POSITION: aead::TagPosition = aead::TagPosition::Postfix;
}

impl AeadCore for Aegis256X2<32> {
    type NonceSize = U32;
    type TagSize = U32;
    const TAG_POSITION: aead::TagPosition = aead::TagPosition::Postfix;
}

impl<const TAG_SIZE: usize> KeySizeUser for Aegis256X2<TAG_SIZE> {
    type KeySize = U32;
}

impl<const TAG_SIZE: usize> KeyInit for Aegis256X2<TAG_SIZE> {
    fn new(key: &Array<u8, Self::KeySize>) -> Self {
        Self { key: *key }
    }
}

impl AeadInOut for Aegis256X2<16> {
...
impl AeadInOut for Aegis256X2<32> {
    fn encrypt_inout_detached(
        &self,
        nonce: &Nonce<Self>,
        associated_data: &[u8],

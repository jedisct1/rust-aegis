...
// Must stay in sync with the bundled aegis128x2.h.
const _: () = {
    assert!(core::mem::size_of::<aegis128x2_state>() == 448);
    assert!(core::mem::align_of::<aegis128x2_state>() == 64);
    assert!(core::mem::size_of::<aegis128x2_mac_state>() == 704);
    assert!(core::mem::align_of::<aegis128x2_mac_state>() == 64);
};
...
extern "C" {
    ...
    fn aegis128x2_stream(out: *mut u8, len: usize, npub: *const u8, k: *const u8);

    fn aegis128x2_stream_xor(
        out: *mut u8,
        input: *const u8,
        len: usize,
        npub: *const u8,
...
impl<const TAG_BYTES: usize> Aegis128X2<TAG_BYTES> {
    ...
    fn state_init(&self, ad: &[u8]) -> aegis128x2_state {
        check_ad_length(ad);
        let mut st = MaybeUninit::<aegis128x2_state>::uninit();
        unsafe {
            aegis128x2_state_init(
                st.as_mut_ptr(),
                ad.as_ptr(),
                ad.len(),
                self.nonce.as_ptr(),
...
impl<const TAG_BYTES: usize> Encryptor<TAG_BYTES> {
    ...
    pub fn finalize(mut self) -> Tag<TAG_BYTES> {
        let mut tag = [0u8; TAG_BYTES];
        let rc =
            unsafe { aegis128x2_state_encrypt_final(&mut self.st, tag.as_mut_ptr(), TAG_BYTES) };
    ...
    fn raw_update(&mut self, ciphertext: *mut u8, plaintext: *const u8, len: usize) {
        self.mlen.add(len);
        let rc =
            unsafe { aegis128x2_state_encrypt_update(&mut self.st, ciphertext, plaintext, len) };
        assert_eq!(rc, 0, "unexpected libaegis encryption failure");
    }
...
impl<'a, const TAG_BYTES: usize> Decryptor<'a, TAG_BYTES> {
    ...
    pub fn update(&mut self, ciphertext: &[u8]) -> Result<(), Error> {
        ...
        let rc = unsafe {
            aegis128x2_state_decrypt_update(
                &mut self.st,
    ...
    pub fn finalize(mut self, tag: &Tag<TAG_BYTES>) -> Result<&'a mut [u8], Error> {
        let rc = unsafe { aegis128x2_state_decrypt_final(&mut self.st, tag.as_ptr(), TAG_BYTES) };
        if rc != 0 {
            return Err(Error::InvalidTag);
        }

...
// Must stay in sync with the bundled aegis256.h.
const _: () = {
    assert!(core::mem::size_of::<aegis256_state>() == 192);
    assert!(core::mem::align_of::<aegis256_state>() == 16);
    assert!(core::mem::size_of::<aegis256_mac_state>() == 288);
    assert!(core::mem::align_of::<aegis256_mac_state>() == 16);
};
...
extern "C" {
    ...
    fn aegis256_state_encrypt_final(st_: *mut aegis256_state, mac: *mut u8, maclen: usize)
        -> c_int;
    ...
    fn aegis256_stream(out: *mut u8, len: usize, npub: *const u8, k: *const u8);

    fn aegis256_stream_xor(
        out: *mut u8,
        input: *const u8,
        len: usize,
        npub: *const u8,
...
impl<const TAG_BYTES: usize> Aegis256<TAG_BYTES> {
    ...
    /// Creates a new AEGIS-256 instance from a key and a nonce.
    ///
    ...
    fn state_init(&self, ad: &[u8]) -> aegis256_state {
        check_ad_length(ad);
...
impl<const TAG_BYTES: usize> Encryptor<TAG_BYTES> {
    ...
    pub fn finalize(mut self) -> Tag<TAG_BYTES> {
        let mut tag = [0u8; TAG_BYTES];
        let rc = unsafe { aegis256_state_encrypt_final(&mut self.st, tag.as_mut_ptr(), TAG_BYTES) };
    ...
    fn raw_update(&mut self, ciphertext: *mut u8, plaintext: *const u8, len: usize) {
        self.mlen.add(len);
        let rc = unsafe { aegis256_state_encrypt_update(&mut self.st, ciphertext, plaintext, len) };
        assert_eq!(rc, 0, "unexpected libaegis encryption failure");
    }
...
impl<'a, const TAG_BYTES: usize> Decryptor<'a, TAG_BYTES> {
    ...
    pub fn finalize(mut self, tag: &Tag<TAG_BYTES>) -> Result<&'a mut [u8], Error> {
        let rc = unsafe { aegis256_state_decrypt_final(&mut self.st, tag.as_ptr(), TAG_BYTES) };
        if rc != 0 {
            return Err(Error::InvalidTag);
        }

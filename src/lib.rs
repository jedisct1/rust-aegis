...
mod tests {
    ...
    fn test_aegis() {
        ...
        let (c, tag) = Aegis128L::new(key, &nonce).encrypt(m, ad);
        let expected_c = [
        ...
        let m2 = Aegis128L::new(key, &nonce).decrypt(&c, &tag, ad).unwrap();
        assert_eq!(m2, m);
    }
    ...
    fn test_aegis_in_place() {
        ...
        let mut mc = m.to_vec();
        let tag = Aegis128L::new(key, &nonce).encrypt_in_place(&mut mc, ad);
        let expected_mc = [
            137, 147, 98, 134, 30, 108, 100, 90, 185, 139, 110, 255, 169, 201, 98, 232, 138, 159,
            166, 71, 169, 80, 96, 205, 2, 109, 22, 101, 71, 138, 231, 79, 130, 148, 159, 175, 131,
            148, 166, 200, 180, 159, 139, 138, 80, 104, 188, 50, 89, 53, 204, 111, 12, 212, 196,
